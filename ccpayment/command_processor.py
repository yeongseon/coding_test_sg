    ...
    def process(self, commands):
        for command in commands:
            if command['command'] == "Add":
                if self.cards.get(command['name']) == None:
            ...
            if command['command'] == "Charge":
                ...
                if card['validation_result'] == False:
                    print("{} {} {} validation result False".format(command['name'], command['command'], command['amount']))
                    continue

                # 한도액을 넘어서면 결제 거부가 되고 무시됨
                if card['limit'] >= card['amount'] + command['amount']:
                    card['amount'] += command['amount']
                    print("{} {} {} ok".format(card['name'], command['command'], command['amount']))
                else:
                    print("{} {} {} ignored limit:{}".format(card['name'], command['command'], command['amount'], card['limit']))
            ...
            if command['command'] == "Credit":
                ...
                card['amount'] -= command['amount']
                print("{} {} {} ok".format(card['name'], command['amount'], command['command']))
